...
export default class Verificador implements MigBotCommand {
    ...
    private roles = BotConfig.config.env === 'production' ? BotConfig.roles.cb_real : BotConfig.roles.cb_pruebas;
    ...
    private bonobotlogo = new Discord.MessageAttachment(`${getHostUrl()}/img/cb-logo.png`, 'bb-logo.png');
    private imgParla = new Discord.MessageAttachment(`${getHostUrl()}/img/foro_img_horizontal.jpeg`, 'foro-img.jpg');
    ...
    public async runCommand(args: string[], msgObject: Discord.Message, client: Discord.Client) {
        ...
        if( args.length > 0  ){  
            ...
            if( args[0] === 'here' ){
                ...
                const channelVerificar = await this.getSelectedChannel( msgObject.client, channel_Verificar_ID ) as Discord.TextChannel;
                const channelRoles = await this.getSelectedChannel( msgObject.client, channel_Roles_ID ) as Discord.TextChannel;
                ...
                channelVerificar.client.on('message', message =>{
                    ...
                    if((message.channel as Discord.TextChannel).name === channelVerificar.name){
