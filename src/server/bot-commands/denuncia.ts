...
export default class Denuncia implements MigBotCommand {
    ...
    private  attachFiles(){
            this.migdrplogo = new Discord.MessageAttachment(`${getHostUrl()}/img/migdrp-logo-small-red.png`, 'migdrp-icon.png');
            this.bonobotlogo = new Discord.MessageAttachment(`${getHostUrl()}/img/LOGO_bb_dsicordback.png`, 'bb-logo.png');
    ...
    private crearEmbedDenuncia( message:Discord.Message, denunciado:string, canal:string, regla:string ): Discord.MessageEmbed {
...
\u200B
    ...
    private async compareUserDates( user:UserCmdInfo, msgObject: Discord.Message ): Promise<boolean> {
    ...
    public async runCommand(args: string[], msgObject: Discord.Message, client: Discord.Client) {
        ...
        const channelDenuncias = await this.getSelectedChannel( msgObject.client, channel_ID ) as Discord.TextChannel;
