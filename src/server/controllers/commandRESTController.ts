...
import { CommandData, RESTController } from '../interfaces';

export class CommandRESTController {
  private cases = new CommandUseCases();
  ...
  public POST: RESTController = async (httpRequest) => {
    try {
      ...
      if (!Array.isArray(httpRequest.body)) {
        const { source = {}, ...data }: CommandData = httpRequest.body;
        source.ip = httpRequest.ip;
        source.browser = httpRequest.headers['User-Agent'];
        if (httpRequest.headers['Referrer']) source.referrer = httpRequest.headers['Referrer'];
        const uploaded = (await this.cases.add({ ...data, source })) as CommandData;
        return {
          headers: {
          ...
          body: {
            status: 'ok',
            message: 'The command was successfully registered.',
            data: uploaded,
          },
        };
      } else {
        if (httpRequest.body.length === 0) {
          ...
          for (let n = 0; n < httpRequest.body.length; n++) {
            const { source = {}, ...data }: CommandData = httpRequest.body[n];
            source.ip = httpRequest.ip;
            source.browser = httpRequest.headers['User-Agent'];
            if (httpRequest.headers['Referrer']) source.referrer = httpRequest.headers['Referrer'];
            commands.push({ ...data, source });
          }
          const uploaded = (await this.cases.add(commands)) as CommandData[];
          return {
            headers: {
              'Content-Type': 'application/json',
              'Last-Modified': new Date(Date.now()).toUTCString(),
            ...
            body: {
              ...
              message: `${httpRequest.body.length === 1 ? 'Command was' : 'Commands were'} successfully registered.`,
              data: uploaded,
            },
          };
        }
  ...
  public LIST: RESTController = async (httpRequest) => {
    try {
      ...
      const Query: CommandData = {
        ...
        prefix: httpRequest.query.prefix || httpRequest.body.prefix,
  ...
  public PATCH: RESTController = async (httpRequest) => {
    try {
      ...
      if (!Array.isArray(httpRequest.body)) {
        ...
        source.browser = httpRequest.headers['User-Agent'];
        if (httpRequest.headers['Referrer']) source.referrer = httpRequest.headers['Referrer'];
        const uploaded = (await this.cases.edit({ ...data, source })) as CommandData;
        return {
          headers: {
            'Content-Type': 'application/json',
            'Last-Modified': new Date(Date.now()).toUTCString(),
          ...
          body: {
            status: 'ok',
            message: 'The command was successfully edited.',
            data: uploaded,
          },
        };
      } else {
        if (httpRequest.body.length === 0) {
          ...
          const uploaded = (await this.cases.edit(commands)) as CommandData[];
          return {
            headers: {
              'Content-Type': 'application/json',
              'Last-Modified': new Date(Date.now()).toUTCString(),
            ...
            body: {
              status: 'ok',
              count: uploaded.length,
              message: `${httpRequest.body.length === 1 ? 'Command was' : 'Commands were'} successfully edited.`,
              data: uploaded,
            },
          };
        }
    ...
    try {
      ...
      if (deleted === 0) {
        return {
          ...
          body: {
            status: 'ok',
            message: 'Command do not exist on database.',
            data: deleted,
          },
        };
      } else {
        return {
          ...
          body: {
            status: 'ok',
            message: 'Command successfully deleted from database.',
            data: deleted,
          },
        };
      }
