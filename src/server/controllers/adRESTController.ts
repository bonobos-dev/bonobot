import { AdUseCases } from '../usecases';
import { AdData, RESTController } from '../interfaces';
import { booleanFromString } from '../utils';

export class AdRESTController {
  private cases = new AdUseCases();
  ...
  public POST: RESTController = async (httpRequest) => {
    try {
      ...
      if (!Array.isArray(httpRequest.body)) {
        const { source = {}, ...data }: AdData = httpRequest.body;
        source.ip = httpRequest.ip;
        source.browser = httpRequest.headers['User-Agent'];
        if (httpRequest.headers['Referrer']) source.referrer = httpRequest.headers['Referrer'];
        const uploaded = (await this.cases.add({ ...data, source })) as AdData;
        return {
          headers: {
          ...
          body: {
            status: 'ok',
            message: 'The ad was successfully registered.',
            data: uploaded,
          },
        };
      } else {
        if (httpRequest.body.length === 0) {
          ...
          for (let n = 0; n < httpRequest.body.length; n++) {
            const { source = {}, ...data }: AdData = httpRequest.body[n];
            source.ip = httpRequest.ip;
            source.browser = httpRequest.headers['User-Agent'];
            if (httpRequest.headers['Referrer']) source.referrer = httpRequest.headers['Referrer'];
            ads.push({ ...data, source });
          }
          const uploaded = (await this.cases.add(ads)) as AdData[];
          return {
            headers: {
              'Content-Type': 'application/json',
              'Last-Modified': new Date(Date.now()).toUTCString(),
            ...
            body: {
              ...
              message: `${httpRequest.body.length === 1 ? 'Ad was' : 'Ads were'} successfully registered.`,
              data: uploaded,
            },
          };
        }
  ...
  public LIST: RESTController = async (httpRequest) => {
    try {
      ...
      let staticHourAttr: boolean;
      let staticDateAttr: boolean;

      if (httpRequest.query.active) staticHourAttr = booleanFromString(httpRequest.query.staticHour);
      if (httpRequest.body.active) staticHourAttr = booleanFromString(httpRequest.body.staticHour);
      if (httpRequest.query.selectable) staticDateAttr = booleanFromString(httpRequest.query.staticDate);
      if (httpRequest.body.selectable) staticDateAttr = booleanFromString(httpRequest.body.staticDate);
      ...
      const Query: AdData = {
        ...
        hour: httpRequest.query.hour || httpRequest.body.hour,
        role: httpRequest.query.role || httpRequest.body.role,
        category: httpRequest.query.category || httpRequest.body.category,
        staticHour: staticHourAttr,
        staticDate: staticDateAttr,
  ...
  public PATCH: RESTController = async (httpRequest) => {
    try {
      ...
      if (!Array.isArray(httpRequest.body)) {
        ...
        source.browser = httpRequest.headers['User-Agent'];
        if (httpRequest.headers['Referrer']) source.referrer = httpRequest.headers['Referrer'];
        const uploaded = (await this.cases.edit({ ...data, source })) as AdData;
        return {
          headers: {
            'Content-Type': 'application/json',
            'Last-Modified': new Date(Date.now()).toUTCString(),
        ...
        if (httpRequest.body.length === 0) {
          ...
          const uploaded = (await this.cases.edit(ads)) as AdData[];
          return {
            headers: {
              'Content-Type': 'application/json',
              'Last-Modified': new Date(Date.now()).toUTCString(),
            ...
            body: {
              status: 'ok',
              count: uploaded.length,
              message: `${httpRequest.body.length === 1 ? 'Ad was' : 'Ads were'} successfully edited.`,
              data: uploaded,
            },
          };
        }
    ...
    try {
      ...
      if (deleted === 0) {
        return {
          ...
          body: {
            status: 'ok',
            message: 'Ad do not exist on database.',
            data: deleted,
          },
        };
      } else {
        return {
          ...
          body: {
            status: 'ok',
            message: 'Ad successfully deleted from database.',
            data: deleted,
          },
        };
      }
