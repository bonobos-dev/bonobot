...
export class MigdrpDbAdapter {
	...
	public async insertOne( { id: _id = Id.makeId(), ...data }, collection:string ){
		try{
			const db = await this.DB.initDb();
	...
	public async updateOne( { id: _id, ...data }:{ id:string }, collection:string ){
		try{
			const db = await this.DB.initDb();
			const result = await db.collection(collection).updateOne({ _id }, { $set: { ...data } });
